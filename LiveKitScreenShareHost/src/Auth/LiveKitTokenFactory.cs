...
namespace LiveKitScreenShareHost.Auth;

internal static class LiveKitTokenFactory
{
    public static string CreatePublisherToken(AppOptions options)
    {
    ...
    public static string CreateViewerToken(AppOptions options)
    {
        return CreateJoinToken(
            options,
            identity: $"{options.Identity}-viewer",
            participantName: $"{options.ParticipantName} Viewer",
            canPublish: false,
            canSubscribe: true);
    }
