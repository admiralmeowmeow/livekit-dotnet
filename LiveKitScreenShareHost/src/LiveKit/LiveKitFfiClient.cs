...
internal sealed class LiveKitFfiClient : IDisposable
{
    ...
    public LiveKitFfiClient(IReadOnlyList<string> libraryCandidates)
    {
        ...
        Native.livekit_ffi_initialize(_callback, captureLogs: true, sdk: "dotnet-screenshare-host", sdkVersion: "0.1.0");
    }
    ...
    public async Task<FfiEvent> WaitForEventAsync(Func<FfiEvent, bool> predicate, CancellationToken cancellationToken)
    {
        ThrowIfDisposed();
        var inbox = _eventInbox ?? throw new InvalidOperationException("FFI inbox is unavailable.");
        return await inbox.WaitForAsync(predicate, cancellationToken);
    }
    ...
    private static void RegisterResolver(IReadOnlyList<string> libraryCandidates)
    {
        lock (ResolverLock)
        {
            ...
            NativeLibrary.SetDllImportResolver(typeof(Native).Assembly, (_, _, _) =>
            {
