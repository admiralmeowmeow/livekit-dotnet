...
public sealed class SwapChainPanelHost : IDisposable
{
    ...
    public IntPtr RenderTargetView => _renderTargetView != IntPtr.Zero
        ? _renderTargetView
        : throw new InvalidOperationException("Render target view has not been initialized.");
    ...
    {
        var (width, height) = GetSwapChainSize(preferredWidth, preferredHeight);
        ...
        if (_swapChain == IntPtr.Zero)
        {
            var description = new SwapChainDescription1
            {
                ...
                BufferUsage = LiveKitD3D11Interop.DxgiUsageRenderTargetOutput,
            ...
            _swapChain = LiveKitD3D11Interop.CreateSwapChainForComposition(_deviceManager.Factory, _deviceManager.Device, description);
        ...
        {
            LiveKitD3D11Interop.Release(ref _renderTargetView);
            LiveKitD3D11Interop.ResizeBuffers(_swapChain, PreferredBufferCount, width, height, DxgiFormat.R8G8B8A8UNorm, 0);
            _bufferWidth = width;
            _bufferHeight = height;
            CreateRenderTargetView();
        }
    ...
    public void Present()
    {
        ...
        var hr = LiveKitD3D11Interop.TryPresent(_swapChain, 0, 0);
        if (hr >= 0)
        {
            return;
        }
        ...
        {
            hr = LiveKitD3D11Interop.TryPresent(_swapChain, 1, 0);
        }

        LiveKitD3D11Interop.ThrowIfFailed(hr, "IDXGISwapChain::Present");
    }

    public void Dispose()
    {
        LiveKitD3D11Interop.Release(ref _renderTargetView);
        LiveKitD3D11Interop.Release(ref _swapChain);
    }

    private (uint Width, uint Height) GetSwapChainSize(uint preferredWidth, uint preferredHeight)
    {
        var width = Volatile.Read(ref _panelPixelWidth);
        var height = Volatile.Read(ref _panelPixelHeight);
        if (width == 0 || height == 0)
        {
            width = Math.Max(1u, preferredWidth);
            height = Math.Max(1u, preferredHeight);
        }
    ...
    {
        var backBuffer = LiveKitD3D11Interop.GetBuffer(_swapChain, 0, LiveKitD3D11Interop.IidD3D11Texture2D);
        try
        {
            _renderTargetView = LiveKitD3D11Interop.CreateRenderTargetView(_deviceManager.Device, backBuffer);
        }
        finally
        {
            LiveKitD3D11Interop.Release(ref backBuffer);
        }
    }
    ...
    private void AttachToPanel(IntPtr swapChain)
    {
        ...
        var unknownPointer = winrtObject.NativeObject.ThisPtr;
